    ...
    DisplayObject3D.prototype.init = function(container) {
        ...
        if (!container.querySelector('ul')) {
            for (i = 0; i < this.pointsArray.length; i++) {
                container.insertAdjacentHTML('beforeend', '<b id="item' + i + '"\u003e+\u003c/b\u003e');
            }
        }
    };
    ...
    Scene3D.prototype.renderCamera = function(camera) {
        ...
        for (var i = 0; i < this.sceneItems.length; i++) {
            var obj = this.sceneItems[i].objects[0],
                ...
                screenPoints = this.Transform3DPointsTo2DPoints(obj.pointsArray, axisRotation, camera),
                //does the container have a ul inside of it.
                hasList = (document.getElementById(obj.containerId).getElementsByTagName("ul").length > 0);
            ...
            for (k = 0; k < obj.pointsArray.length; k++) {
                ...
                if (currItem) {
                    ...
                    currItem.style.opacity = (currItem.scale - .5);
                }
            }
        }
    };
